...
public class GetterUtil {
	...
	public static boolean get(String value, boolean defaultValue) {
		...
		try {
			...
			return value.equals(BOOLEANS[0]) || value.equals(BOOLEANS[1]) || value.equals(BOOLEANS[2]) || value.equals(
					BOOLEANS[3]) || value.equals(BOOLEANS[4]) || value.equals(BOOLEANS[5]);
	...
	private static String _trim(String value) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (Character.isDigit(c) || ((c == CharPool.DASH) && ((i == 0) || (value.charAt(
					i - 1) == CharPool.UPPER_CASE_E) || (value.charAt(
							i - 1) == CharPool.LOWER_CASE_E))) || (c == CharPool.PERIOD) || (c == CharPool.UPPER_CASE_E) || (c == CharPool.LOWER_CASE_E)) {
